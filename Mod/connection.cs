    ...
    public static class ConnectionManager
    {
        ...
        public static void Initialize(string host = "127.0.0.1", int port = 5000)
        {
            ...
            try
            {
                ...
                ModContent.GetInstance<RL_API>().Logger.Info($"[RL/CM] Socket Connected to {host}:{port}");

                Task.Run(() => IoLoop());
            }
            catch (Exception ex)
            {
                ModContent.GetInstance<RL_API>().Logger.Error($"[RL/CM] Socket Connection error: {ex.Message}");
            }
        }
